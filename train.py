...
class Train:
    ...
    file_pc = "D:/Knowledge-Distillation/GCN-with-Hinton-Knowledge-Distillation-main/.checkpoints/"
    # "D:\Knowledge-Distillation\GCN-with-Hinton-Knowledge-Distillation-main\.checkpoints"
    def save_checkpoint(self, filename=file_pc+args.name, ts='teacher'):
    ...
    def load_checkpoint(self, filename=file_pc+args.name, ts='teacher'):
        print('Load {ts} model...'.format(ts=ts))
        if ts == 'teacher':
            load_state = torch.load(filename+'_teacher')
            ...
            load_state = torch.load(filename+'_student')
